...
func ProcessLibraries(clangBin, outDir, extraLibsDir string) {
	...
	generate(
		...
		clangBin,
		"--std=c++17 "+pkgConfigCflags("Qt6Core"),
		outDir,
		"include/cbor",
		ClangMatchSameHeaderDefinitionOnly,
		&headerList,
	...
	*/
	...
	/*
		...
		/*
			...
			// Depends on QtCore/Gui/Widgets
			generate(
				"src/qt/restricted-extras-charts",
				[]string{
			...
			generate(
				"src/qt/restricted-extras-qscintilla",
				[]string{
	...
	*/
	...
	/*
		...
		zigCLibPath := filepath.Join(outDir, "src/qt", "libqtc.zig")
		...
		err = os.WriteFile(filepath.Join(outDir, "src/qt", "libqt6c.h"), []byte(qt6cHeader), 0644)
		if err != nil {
			panic(err)
		}
		...
		qtLibHeader := filepath.Join(outDir, "src/qt", "qtlibc.h")
		...
		zigQtPath := filepath.Join(outDir, "src/qt", "libqt6.zig")
		...
		threadingQtLibHeader := filepath.Join(outDir, "src/qt", "threading", "libqt6zigthreading.h")
		...
		zigThreadingQtLib := filepath.Join(outDir, "src/qt", "threading", "libqt6zigthreading.zig")
